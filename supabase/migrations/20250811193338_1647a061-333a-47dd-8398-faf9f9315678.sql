...
create table if not exists public.insumo_recipes (
...
create table if not exists public.insumo_recipe_items (
  id uuid primary key default gen_random_uuid(),
...
create policy if not exists insumo_recipes_select_own on public.insumo_recipes
for select using (auth.uid() = user_id);

create policy if not exists insumo_recipes_insert_own on public.insumo_recipes
for insert with check (auth.uid() = user_id);

create policy if not exists insumo_recipes_update_own on public.insumo_recipes
for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy if not exists insumo_recipes_delete_own on public.insumo_recipes
for delete using (auth.uid() = user_id);

-- Policies for items: only through own recipes
create policy if not exists insumo_items_select_own on public.insumo_recipe_items
for select using (exists (
  select 1 from public.insumo_recipes r where r.id = recipe_id and r.user_id = auth.uid()
));

create policy if not exists insumo_items_insert_own on public.insumo_recipe_items
for insert with check (exists (
  select 1 from public.insumo_recipes r where r.id = recipe_id and r.user_id = auth.uid()
));

create policy if not exists insumo_items_update_own on public.insumo_recipe_items
...
create policy if not exists insumo_items_delete_own on public.insumo_recipe_items
